        ...
        const response = await (0, req_1.req)(url, {
            ...
            hooks: {
                beforeRetry({ response, error, attempt }) {
                    logger.log(`Upload of ${name} resource will be retried due to ${error ? `an error with message "${error.message}"` : `unexpected status ${response.statusText}(${response.status})`} in previous attempt (${attempt})`);
                },
            },
        });
